...
import { getDealDocumentStatus, getPipelineDocumentSummary } from "@/lib/pipeline/dealDocuments";
import {
  GripVertical,
  ChevronRight,
  Target,
...
function DocumentBadges({ deal, hasTechnicalProposal }: { deal: Quotation; hasTechnicalProposal: boolean }) {
  const docs = getDealDocumentStatus(deal, new Set(hasTechnicalProposal ? [deal.id] : []));
  return (
    <div className="flex flex-wrap gap-1 mb-2">
      <span className={`inline-flex items-center gap-0.5 text-[9px] font-semibold px-1.5 py-0.5 rounded-sm border ${
        docs.proformaSent ? "text-teal-700 bg-teal-50 border-teal-100" : "text-gray-500 bg-gray-50 border-gray-100"
      }`}>
        <FileText className="w-2.5 h-2.5" />
        {docs.proformaSent ? "Proforma enviada" : "Proforma rascunho"}
      </span>
      <span className={`inline-flex items-center gap-0.5 text-[9px] font-semibold px-1.5 py-0.5 rounded-sm border ${
        hasTechnicalProposal ? "text-blue-700 bg-blue-50 border-blue-100" : "text-amber-700 bg-amber-50 border-amber-100"
      }`}>
        <ClipboardList className="w-2.5 h-2.5" />
        {hasTechnicalProposal ? "Prop. técnica" : "Sem prop. técnica"}
      </span>
      {docs.hasProformaPdf && (
        <span className="inline-flex items-center gap-0.5 text-[9px] font-semibold px-1.5 py-0.5 rounded-sm border text-emerald-700 bg-emerald-50 border-emerald-100">
          <FileCheck className="w-2.5 h-2.5" /> PDF
        </span>
      )}
    </div>
  );
...
function KanbanCard({
  ...
  return (
    ...
    >
        ...
        <div className="relative">
          ...
          {showMenu && (
            <div className="absolute right-0 top-full w-44 bg-white border border-gray-200 rounded-md shadow-xl z-20 py-1">
              ...
              <button onClick={() => router.push(`/dashboard/proposals/new?quotationId=${deal.id}`)} className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                <ClipboardList className="w-3.5 h-3.5" /> {hasTechnicalProposal ? "Editar Prop. Técnica" : "Criar Prop. Técnica"}
              </button>
              <button onClick={() => router.push(`/dashboard/pdf-preview/${deal.id}`)} className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                <FileText className="w-3.5 h-3.5" /> PDF Proforma
              </button>
      ...
      <DocumentBadges deal={deal} hasTechnicalProposal={hasTechnicalProposal} />
        ...
        {deals.length === 0 ? (
          ...
          deals.map((deal) => (
            <KanbanCard 
              ...
              hasTechnicalProposal={proposalQuotationIds.has(deal.id)}
...
export default function PipelinePage() {
  const { quotations, fetchQuotations, setNextActionFull, commercialProposals, fetchCommercialProposals } = useQuotationsStore();
  ...
  const [filter, setFilter] = useState<"all" | "high" | "negotiation" | "inactive" | "no_action" | "expiring" | "follow_up" | "docs_missing">("all");

  useEffect(() => {
    fetchQuotations();
    fetchClients();
    fetchCommercialProposals();
  }, [fetchQuotations, fetchClients, fetchCommercialProposals]);

  const proposalQuotationIds = useMemo(
    () => new Set(commercialProposals.map((p) => p.quotation_id)),
    [commercialProposals]
  );

  const docSummary = useMemo(
    () => getPipelineDocumentSummary(quotations, proposalQuotationIds),
    [quotations, proposalQuotationIds]
  );
  ...
  const missingTechnicalProposal = useMemo(() => activeQuotations.filter(q => {
    const stage = q.pipeline_stage || "";
    return ["proposal", "negotiation"].includes(stage) && !proposalQuotationIds.has(q.id);
  }), [activeQuotations, proposalQuotationIds]);
  const highPriorityInactive = useMemo(() => activeQuotations.filter(q => q.priority === "high" && daysSince(q.updated_at || q.created_at) > 3), [activeQuotations]);

  const totalAlerts = noNextAction.length + expiringSoon.length + inactiveDeals.length + highPriorityInactive.length + missingTechnicalProposal.length;
  ...
  const handleDrop = async (e: React.DragEvent, stage: PipelineStage) => {
    ...
    try {
      ...
      await fetchCommercialProposals();
    } catch (err: any) {
  ...
  const filteredQuotations = quotations.filter(q => {
    ...
    if (filter === "docs_missing") return getDealDocumentStatus(q, proposalQuotationIds).needsAttention;
    return true;
  });
  ...
  return (
    <div className="animate-in fade-in duration-500 flex flex-col h-full min-h-[calc(100vh-80px)]">
      ...
      {(docSummary.missingTechnical > 0 || docSummary.notSent > 0 || docSummary.missingPdf > 0) && (
        <div className="mb-6 flex flex-wrap gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl">
          <span className="text-xs font-bold text-slate-600 uppercase tracking-wider w-full mb-1">Sincronização de Documentos</span>
          {docSummary.missingTechnical > 0 && (
            <button onClick={() => setFilter("docs_missing")} className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-full hover:bg-blue-100 transition-colors">
              <FileX className="w-3.5 h-3.5" /> {docSummary.missingTechnical} sem proposta técnica
            </button>
          )}
          {docSummary.notSent > 0 && (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-100 px-3 py-1.5 rounded-full">
              <FileText className="w-3.5 h-3.5" /> {docSummary.notSent} proforma não enviada
            </span>
          )}
          {docSummary.missingPdf > 0 && (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-700 bg-rose-50 border border-rose-100 px-3 py-1.5 rounded-full">
              <AlertCircle className="w-3.5 h-3.5" /> {docSummary.missingPdf} PDF em falta
            </span>
          )}
        </div>
      )}
            ...
            {totalAlerts === 0 ? (
              ...
              <>
                {noNextAction.map(deal => (
                  <div key={`no-action-${deal.id}`} className="flex items-center justify-between p-2 bg-red-50/50 rounded-lg">
                    <div className="flex items-start gap-2 overflow-hidden">
                ...
                {expiringSoon.map(deal => (
                  <div key={`expiring-${deal.id}`} className="flex items-center justify-between p-2 hover:bg-amber-50 rounded-lg">
                    <div className="flex items-start gap-2 overflow-hidden">
                ...
                {missingTechnicalProposal.map(deal => (
                  <div key={`missing-tech-${deal.id}`} className="flex items-center justify-between p-2 bg-blue-50/60 rounded-lg">
                    <div className="flex items-start gap-2 overflow-hidden">
                      <ClipboardList className="w-4 h-4 text-blue-500 shrink-0 mt-0.5" />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-blue-900 truncate">Proposta técnica em falta</p>
                        <p className="text-xs text-blue-700/70 truncate">{deal.client_name} • {deal.quotation_number}</p>
                      </div>
                    </div>
                    <Link href={`/dashboard/proposals/new?quotationId=${deal.id}`} className="text-xs font-bold text-blue-600 bg-blue-100 hover:bg-blue-200 px-2 py-1 rounded">Criar</Link>
                  </div>
                ))}
                {inactiveDeals.map(deal => (
                  <div key={`inactive-${deal.id}`} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg">
                    <div className="flex items-start gap-2 overflow-hidden">
                      <AlertTriangle className="w-4 h-4 text-gray-500 shrink-0 mt-0.5" />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate">Parado há {daysSince(deal.updated_at || deal.created_at)} dias</p>
                        <p className="text-xs text-gray-500 truncate">{deal.client_name} • {deal.quotation_number}</p>
                      </div>
                    </div>
                    <button onClick={() => setActionModal(deal)} className="text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded">Agir</button>
                  </div>
                ))}
                {highPriorityInactive.map(deal => (
                  <div key={`high-priority-${deal.id}`} className="flex items-center justify-between p-2 bg-orange-50/60 rounded-lg">
                    <div className="flex items-start gap-2 overflow-hidden">
                      <Zap className="w-4 h-4 text-orange-500 shrink-0 mt-0.5" />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-orange-900 truncate">Alta prioridade sem movimento</p>
                        <p className="text-xs text-orange-700/70 truncate">{deal.client_name} • {formatCurrency(deal.grand_total)}</p>
                      </div>
                    </div>
                    <button onClick={() => setActionModal(deal)} className="text-xs font-bold text-orange-600 bg-orange-100 hover:bg-orange-200 px-2 py-1 rounded">Follow-up</button>
                  </div>
                ))}
              </>
            )}
      ...
      <div className="flex overflow-x-auto gap-2 pb-2 mb-4 scrollbar-hide">
        ...
        <button 
          onClick={() => setFilter("docs_missing")}
          className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5 ${filter === "docs_missing" ? "bg-blue-100 text-blue-800 border border-blue-200" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}
        >
          <FileX className="w-3.5 h-3.5" /> Docs em Falta
        </button>
        <button 
          onClick={() => setFilter("inactive")}
          className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5 ${filter === "inactive" ? "bg-gray-200 text-gray-800 border border-gray-300" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}
        >
          <Clock className="w-3.5 h-3.5" /> Parados +7d
        </button>
      </div>
          ...
          {PIPELINE_STAGES.map((stage) => {
            ...
            return (
              <KanbanColumn
                ...
                proposalQuotationIds={proposalQuotationIds}
